...
export default function PartnerEsimOrdersPage() {
  const { t } = useLanguage()
  ...
  useEffect(() => {
    ...
    setLoading(true)
    fetch(`/api/agent/esim-orders?agent_id=${selectedAgentId}`)
      .then(r => r.json())
    ...
    <div className="flex flex-col h-screen overflow-hidden bg-gray-50 dark:bg-gray-950">
      ...
      <div className="flex-1 overflow-y-auto p-6">
        <div className="flex items-center justify-between mb-6">
          ...
          {selectedAgentId && (
            <Link
              href={`/admin/partner-esim-orders/create?agent_id=${selectedAgentId}`}
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50 dark:bg-gray-800 text-gray-500 dark:text-gray-400 text-xs">
                <tr>
                  ...
                  <th className="text-left px-4 py-3 font-medium">採購訂單號</th>
                  ...
                  return (
                    <tr key={order.id} className="bg-white dark:bg-gray-900 hover:bg-gray-50 dark:hover:bg-gray-800/50">
                      <td className="px-4 py-3">
                        ...
                        {order.bc_order_id && <div className="text-xs text-gray-400 mt-0.5">BC: {order.bc_order_id}</div>}
                      </td>
                      <td className="px-4 py-3 text-xs text-gray-500 font-mono">
                        {order.partner_order_id ? '已關聯' : '—'}
                      </td>
