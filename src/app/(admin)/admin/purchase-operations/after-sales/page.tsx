...
function DetailModal({ row, onClose }: { row: AfterSaleRow; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    fetch('/api/admin/operations/after-sales', {
      method: 'POST',
...
export default function AfterSalesPage() {
  ...
  const doSearch = useCallback(async (p = 1) => {
    ...
    try {
      const res = await fetch(`/api/admin/operations/after-sales?${params}`)
  ...
  }, [channelOrderId, bcAfterSaleId, iccid, status, dateFrom, dateTo])
