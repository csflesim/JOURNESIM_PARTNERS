...
function DetailsModal({ row, onClose }: { row: PlanRow; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/admin/operations/plan-details?${params}`)
      .then(r => r.json())
...
function UsageModal({ row, onClose }: { row: PlanRow; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/admin/operations/plan-usage?${params}`)
      .then(r => r.json())
      .then(d => {
...
function TrafficModal({ row, onClose }: { row: PlanRow; onClose: () => void }) {
  ...
  function query() {
    ...
    fetch(`/api/admin/operations/daily-traffic?${params}`)
      .then(r => r.json())
      .then(d => {
        if (d.error) setError(d.error)
...
export default function PlanQueryPage() {
  ...
  const doSearch = useCallback(async (p = 1) => {
    ...
    try {
      const res = await fetch(`/api/admin/operations/plan-query?${params}`)
  ...
  }, [iccid, bcOrderId, planStatus, dateFrom, dateTo])
