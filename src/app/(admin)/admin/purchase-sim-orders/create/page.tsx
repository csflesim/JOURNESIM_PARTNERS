...
import { ChevronDown, ChevronRight, Loader2, Search, X, Check, FileText, ChevronUp } from 'lucide-react'
...
// 號段：BigInt 產生連號 ICCID（最多 10000）
function generateRange(start: string, end: string): string[] {
  try {
    const s = BigInt(start.trim())
    const e = BigInt(end.trim())
    if (e < s) return []
    const count = Number(e - s) + 1
    if (count > 10000) return []
    const len = start.trim().length
    const result: string[] = []
    for (let i = s; i <= e; i++) result.push(i.toString().padStart(len, '0'))
    return result
  } catch {
    return []
  }
...
// 文本：逗號 / 換行 分隔，去重去空
function parseTextIccids(text: string): string[] {
  return [...new Set(
        ...
        <div className="absolute top-full left-0 right-0 mt-1 z-50 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-lg overflow-hidden">
          <div className="p-2 border-b border-gray-100 dark:border-gray-700">
            <div className="relative">
              ...
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="搜索國家..." autoFocus
            ...
            {filtered.length === 0 ? (
              ...
              return (
                <button key={opt.value} type="button" onClick={() => toggle(opt.value)}
                  className={clsx('w-full text-left px-4 py-2 text-sm flex items-center gap-2 transition-colors',
  ...
  return (
    <div className="relative">
      <select value={value} onChange={e => onChange(e.target.value)}
        className={clsx('w-full appearance-none px-4 py-3 pr-10 rounded-xl border text-sm transition-colors focus:outline-none',
          'border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800',
...
function DetailModal({ product, onClose }: { product: Product; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={onClose}>
      <div className="bg-gray-100 dark:bg-gray-900 rounded-2xl shadow-xl w-full max-w-lg max-h-[88vh] flex flex-col" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex-1 overflow-y-auto px-4 pb-4 space-y-3">
          <div className="bg-white dark:bg-gray-800 rounded-xl px-5 py-4 space-y-3">
            ...
            <Row label="計費類型" value={product.plan_type === '0' ? '總量型' : product.plan_type === '1' ? '單日型' : '—'} />
            ...
            <Row label="熱點分享" value={product.hotspot_support === '1' ? '支援' : product.hotspot_support === '0' ? '不支援' : '—'} />
          </div>
              ...
              <div className="space-y-2">
                {product.prices.map(tier => (
                  <div key={tier.copies} className="flex items-center justify-between text-sm">
                    <span className="text-gray-600 dark:text-gray-300">{parseInt(product.days ?? '1') * parseInt(tier.copies)} 天</span>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl px-5 py-4">
              ...
              <div className="flex flex-wrap gap-1.5">
                {product.countries.map(c => (
                  <span key={c.mcc} className="text-xs bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 px-2 py-0.5 rounded-md">{c.name}</span>
                ))}
              </div>
            </div>
          )}
        ...
        <div className="px-6 py-4 border-t border-gray-200 dark:border-gray-700 flex justify-end flex-shrink-0">
          <button onClick={onClose} className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-sm">關閉</button>
        </div>
      </div>
    </div>
  )
...
function ProductTable({ products, filterDays, selectedProduct, selectedCopies, onSelect, emptyText }: {
  products: Product[]
  filterDays: string
    ...
    <>
      ...
      <div className="border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {products.length === 0 ? (
              <tr><td colSpan={COLS} className="text-center py-12 text-gray-400 text-sm">{emptyText}</td></tr>
            ) : products.map(p => {
              ...
              const detailCell = (
                <td className="px-4 py-3 text-center" onClick={e => e.stopPropagation()}>
                  <button onClick={() => setDetailProduct(p)} className="p-1.5 rounded hover:bg-indigo-50 dark:hover:bg-indigo-900/30 text-indigo-500 dark:text-indigo-400">
                    <FileText size={14} />
                  </button>
                </td>
              )
              ...
              if (filterDays && matchingTier) {
                ...
                return (
                  <tr key={p.sku_id} onClick={() => onSelect(p, tierCopies)}
                    className={clsx('border-t border-gray-100 dark:border-gray-800 cursor-pointer transition-colors',
                      isSelected && selectedCopies === tierCopies ? 'bg-indigo-50 dark:bg-indigo-900/20' : 'bg-white dark:bg-gray-900 hover:bg-gray-50 dark:hover:bg-gray-800/50')}>
                    ...
                    {capacityCell}{speedCell}{countryCell}
                    <td className="px-4 py-3 text-center font-medium text-gray-700 dark:text-gray-200">{totalDays} 天</td>
                    <td className="px-4 py-3 text-right font-medium text-gray-800 dark:text-gray-100">¥{parseFloat(matchingTier.settlementPrice).toFixed(2)}</td>
                    {detailCell}
                  </tr>
                )
              }
              ...
              return (
                <React.Fragment key={p.sku_id}>
                  <tr className={clsx('border-t border-gray-100 dark:border-gray-800 transition-colors',
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-start gap-2">
                        <button onClick={e => hasPrices && toggleExpand(p.sku_id, e)}
                          className={clsx('mt-0.5 flex-shrink-0 transition-transform duration-150',
                            expanded ? 'rotate-90' : '',
                    ...
                    {capacityCell}{speedCell}{countryCell}
                    <td className="px-4 py-3 text-center text-gray-400 text-xs">{hasPrices ? `${p.prices.length} 規格` : (p.days ? `${p.days} 天` : '—')}</td>
                    <td className="px-4 py-3 text-right text-gray-400 text-xs">{hasPrices ? '展開查看' : '—'}</td>
                    {detailCell}
                  </tr>
                  {expanded && hasPrices && p.prices.map(tier => {
                    ...
                    return (
                      <tr key={tier.copies} onClick={() => onSelect(p, tierCopies)}
                        className={clsx('border-t border-gray-100 dark:border-gray-800 cursor-pointer transition-colors',
                          isTierSelected ? 'bg-indigo-50 dark:bg-indigo-900/20' : 'bg-gray-50/60 dark:bg-gray-800/30 hover:bg-indigo-50/50 dark:hover:bg-indigo-900/10')}>
...
export default function CreateSimOrderPage() {
  ...
  const [countryMode, setCountryMode] = useState<'any' | 'all'>('any')
  const [filterDays, setFilterDays] = useState('')
  const [filterCapacity, setFilterCapacity] = useState('')
  const [searchQuery, setSearchQuery] = useState('')

  // Selected product & copies tier
  ...
  useEffect(() => {
    ...
    fetch('/api/admin/products?types=110,111')
      .then(r => r.json())
      .then(prodData => {
  ...
  const tabProducts = useMemo(() => products.filter(p => SIM_TYPES.includes(p.type)), [products])

  const countryOptions = useMemo(() => countriesParams.map(c => ({ value: c.mcc, label: c.name })), [countriesParams])
  ...
  const capacityOptions = useMemo(() => {
    ...
    tabProducts.forEach(p => { if (p.high_flow_size) map.set(p.high_flow_size, p.plan_type === '1') })
    return Array.from(map.entries()).sort(([a], [b]) => parseFloat(a) - parseFloat(b))
      .map(([v, isDaily]) => {
        const base = capacityLabels[v] ?? formatKB(v)
        return { value: v, label: isDaily && !base.includes('/天') ? `${base}/天` : base }
      })
  ...
  const iccids = useMemo(() => {
    if (iccidMode === 'text') return parseTextIccids(iccidText)
  ...
  const totalAmount = unitPrice * iccids.length

  async function handleSubmit() {
    if (!selectedProduct) return
    if (iccids.length === 0) { setSubmitError('請輸入 ICCID'); return }
    setSubmitting(true)
    setSubmitError('')
    try {
      const res = await fetch('/api/admin/sim-orders', {
        method: 'POST',
      ...
      router.push('/purchase-sim-orders')
    } catch (err) {
        ...
        {loadingProducts ? (
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <CountryMultiSelect options={countryOptions} selected={filterCountries} onChange={setFilterCountries} />
          ...
          <div className="space-y-4">
              ...
              <div>
                ...
                <div className="mt-1.5 flex items-center gap-1">
                  ...
                  {(['any', 'all'] as const).map(mode => (
                    <button key={mode} type="button" onClick={() => setCountryMode(mode)}
                      className={clsx('text-xs px-2 py-0.5 rounded-md transition-colors',
                        countryMode === mode ? 'bg-indigo-600 text-white' : 'text-gray-400 hover:text-gray-600 dark:hover:text-gray-300')}>
                      {mode === 'any' ? '模糊' : '精準'}
                    </button>
                  ))}
                </div>
              </div>
              <FilterSelect placeholder="選擇天數" options={daysOptions} value={filterDays} onChange={setFilterDays} />
              <FilterSelect placeholder="選擇流量" options={capacityOptions} value={filterCapacity} onChange={setFilterCapacity} />
            </div>
            ...
            <div>
              <div className="relative">
                ...
                <input value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="搜索套餐名稱或ID"
              ...
              <p className="text-xs text-gray-400 mt-1.5 pl-1">
                {filteredProducts.length > 0 ? `符合條件的套餐：${filteredProducts.length} 個` : '無符合條件的套餐'}
              </p>
            </div>
            ...
            <ProductTable
              ...
              onSelect={(p, c) => { setSelectedProduct(p); setCopies(c) }}
              emptyText={searchQuery || filterCountries.length || filterDays || filterCapacity ? '無符合條件的套餐' : '此分類無套餐資料'}
            />

            {/* Selected product — copies + ICCID input */}
            {selectedProduct && (
              <div className="bg-white dark:bg-gray-900 border border-indigo-200 dark:border-indigo-800 rounded-xl p-5 space-y-5">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {selectedProduct.prices.map(tier => {
                        ...
                        return (
                          <button key={tier.copies} type="button" onClick={() => setCopies(tierCopies)}
                ...
                {/* ICCID 信息 */}
                <div>
                  <div className="flex items-center gap-0 mb-3 border-b border-gray-200 dark:border-gray-700">
                    {(['text', 'range'] as const).map(mode => (
                      <button key={mode} type="button" onClick={() => setIccidMode(mode)}
                  ...
                  {iccidMode === 'text' ? (
                    <div className="bg-gray-50 dark:bg-gray-800 rounded-xl p-4 space-y-3">
                      <div className="text-xs font-medium text-gray-600 dark:text-gray-300">ICCID列表</div>
                      <textarea
                        value={iccidText}
                        onChange={e => setIccidText(e.target.value)}
                        placeholder="請輸入ICCID，多個ICCID用英文逗號或回車分隔"
                        rows={4}
                        className="w-full px-3 py-2 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-sm text-gray-800 dark:text-gray-100 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none"
                      />
                      <div className="flex items-center justify-between text-xs text-gray-400">
                        <span>支持英文逗號(,)或回車換行分隔多個ICCID</span>
                        <span>卡片數量：{iccids.length}</span>
                      </div>
                    </div>
                  ) : (
                    <div className="bg-gray-50 dark:bg-gray-800 rounded-xl p-4 space-y-3">
                      <div className="space-y-3">
                        <div>
                          <label className="block text-xs text-gray-500 dark:text-gray-400 mb-1">起始號段</label>
                          <input value={rangeStart} onChange={e => setRangeStart(e.target.value)} placeholder="請輸入ICCID"
                            className="w-full px-3 py-2 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-sm text-gray-800 dark:text-gray-100 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                        </div>
                        <div>
                          <label className="block text-xs text-gray-500 dark:text-gray-400 mb-1">結束號段</label>
                          <input value={rangeEnd} onChange={e => setRangeEnd(e.target.value)} placeholder="請輸入ICCID"
                            className="w-full px-3 py-2 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-sm text-gray-800 dark:text-gray-100 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                        </div>
                      </div>
                      <div className="flex items-center justify-between text-xs text-gray-400">
                        <span>系統將自動生成從起始號段到結束號段之間的所有ICCID</span>
                        <span>卡片數量：{iccids.length}</span>
                      </div>
                      {rangeStart && rangeEnd && iccids.length === 0 && (
                        <p className="text-xs text-red-500">號段無效或超過 10,000 張上限</p>
                      )}
                    </div>
                  )}
                </div>
                ...
                <div>
                  <button type="button" onClick={() => setShowMore(v => !v)}
                    ...
                    更多選項(選填)
                    {showMore ? <ChevronUp size={14} /> : <ChevronRight size={14} />}
                  </button>
                  {showMore && (
                    <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div>
                        <label className="block text-xs text-gray-500 dark:text-gray-400 mb-1">代理商 ID</label>
                        <input value={agentId} onChange={e => setAgentId(e.target.value)} placeholder="選填"
                          className="w-full px-3 py-2 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-sm text-gray-800 dark:text-gray-100 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                      </div>
                      <div>
                        <label className="block text-xs text-gray-500 dark:text-gray-400 mb-1">備注</label>
                        <input value={comment} onChange={e => setComment(e.target.value)} placeholder="選填"
                          className="w-full px-3 py-2 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-sm text-gray-800 dark:text-gray-100 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                      </div>
                    </div>
                  )}
                ...
                <div className="flex items-center justify-between pt-2 border-t border-gray-100 dark:border-gray-800">
                  <div className="text-sm text-gray-600 dark:text-gray-400">
                    訂單金額：
                    <span className="ml-2 text-base font-semibold text-indigo-600 dark:text-indigo-400">
                      ¥{totalAmount.toFixed(2)}
                    </span>
                    {iccids.length > 0 && (
                      <span className="ml-2 text-xs text-gray-400">（{iccids.length} 張 × ¥{unitPrice.toFixed(2)}）</span>
                    )}
                  </div>
                  <div className="flex items-center gap-3">
                    {submitError && <p className="text-xs text-red-500">{submitError}</p>}
                    <button onClick={handleSubmit} disabled={submitting || iccids.length === 0}
