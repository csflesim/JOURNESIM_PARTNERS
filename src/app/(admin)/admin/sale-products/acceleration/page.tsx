...
export default function SaleProductsAccelPage() {
  const [search, setSearch] = useState('')
  const [products, setProducts] = useState(
    PRODUCTS.filter(p => p.kind === 'accel').map(p => ({ ...p, enabled: true }))
  )
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-50 dark:bg-gray-950">
      <Header breadcrumb={['銷售模組', '商品管理', '加速包']} />
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/50">
                ...
                <th className="text-left px-4 py-3 font-medium text-gray-500 dark:text-gray-400">適用國家／地區</th>
                <th className="text-left px-4 py-3 font-medium text-gray-500 dark:text-gray-400">方案數量</th>
                  ...
                  <>
                    ...
                    >
                      ...
                      <td className="px-4 py-3.5 text-gray-600 dark:text-gray-300">
                        {product.coverageCount > 1
                          ? `${product.coverageCount} 個國家`
                    ...
                    {isExpanded && (
                      <tr key={`${product.id}-plans`} className={clsx(
                        ...
                        <td colSpan={7} className="px-8 py-4">
                          <div className="text-xs font-semibold text-indigo-600 dark:text-indigo-400 mb-2">加速包規格（無期限，用完即止）</div>
                          <div className="flex flex-wrap gap-2">
                            {product.fixedPlans.map(plan => (
                              <div key={plan.id} className="flex items-center gap-1.5 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-1.5 text-xs">
                                <span className="font-medium text-gray-900 dark:text-white">{plan.gb} GB</span>
          ...
          {filtered.length === 0 && (
            <div className="py-16 text-center text-sm text-gray-400">找不到符合的加速包</div>
          )}
        </div>
      </div>
    </div>
