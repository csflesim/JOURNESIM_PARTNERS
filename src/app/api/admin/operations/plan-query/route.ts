...
// GET /api/admin/operations/plan-query
// Params: iccid?, bc_order_id?, plan_status?, date_from?, date_to?, page, limit
export async function GET(req: Request) {
  ...
  // ── Strategy A: ICCID provided → query BC F012 directly ─────────────
  if (iccid) {
    // Call BC F012 for authoritative plan data
    let bcResults: any[] = []
    try {
      const result = await getPlanUsage({ iccid })
      bcResults = Array.isArray(result) ? result : [result]
    } catch {
      // F012 may fail if ICCID not found; fall through to local DB only
    }

    // Also try to find local DB record for order context
    const { data: esimProfile } = await supabase
      .from('esim_profiles')
      .select('order_item_id, order_id')
      .eq('iccid', iccid)
      .maybeSingle()

    const { data: orderItemByArr } = await supabase
      .from('order_items')
      .select('id, order_id')
      .contains('iccid', [iccid])
      .maybeSingle()

    const orderItemId = esimProfile?.order_item_id ?? orderItemByArr?.id ?? null
    const orderId = esimProfile?.order_id ?? orderItemByArr?.order_id ?? null

    // Fetch order + product details from local DB if available
    let localOrder: any = null
    if (orderId) {
      const { data } = await supabase
        .from('orders')
        .select('id, channel_order_id, bc_order_id, order_type, created_at')
        .eq('id', orderId)
        .single()
      localOrder = data
    }
    ...
    if (orderItemId) {
      const { data } = await supabase
        .from('order_items')
        .select('id, channel_sub_order_id, bc_sub_order_id, sku_id, plan_status, plan_start_time, plan_end_time, iccid, products(name, type)')
        .eq('id', orderItemId)
        .single()
      localItem = data
    }

    // If BC returned plan data, use it as source of truth
    if (bcResults.length > 0) {
      ...
      const items = bcOrder.subOrderList?.map((sub: any) => ({
        ...
        has_esim_profile: !!esimProfile,
        matched_iccid: iccid,
        // extra F012 fields for display
      ...
      // Apply plan_status filter if set
    ...
    // BC returned nothing — try local DB only
    if (localItem && localOrder) {
      const product = Array.isArray(localItem.products) ? localItem.products[0] : localItem.products
      const item = {
        id: localItem.id,
        channel_sub_order_id: localItem.channel_sub_order_id,
        bc_sub_order_id: localItem.bc_sub_order_id,
        sku_id: localItem.sku_id,
        sku_name: product?.name ?? localItem.sku_id,
        order_type: localOrder.order_type,
        iccids: localItem.iccid ?? [iccid],
        plan_status: localItem.plan_status,
        plan_start_time: localItem.plan_start_time,
        plan_end_time: localItem.plan_end_time,
        channel_order_id: localOrder.channel_order_id,
        bc_order_id: localOrder.bc_order_id,
        order_created_at: localOrder.created_at,
        has_esim_profile: !!esimProfile,
        matched_iccid: iccid,
      }
  ...
  if (dateTo) {
    query = query.lte('orders.created_at', dateTo + 'T23:59:59Z')
  }
  ...
  if (bcOrderId) {
    const { data: matchedOrders } = await supabase
      .from('orders')
      .select('id')
      .eq('bc_order_id', bcOrderId)
    if (!matchedOrders?.length) return NextResponse.json({ items: [], total: 0 })
    query = query.in('order_id', matchedOrders.map(o => o.id))
  }

  const { data, error, count } = await query
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })

  // Check esim_profiles existence
  ...
  const items = (data ?? []).map(item => {
    ...
    return {
      id: item.id,
      channel_sub_order_id: item.channel_sub_order_id,
      bc_sub_order_id: item.bc_sub_order_id,
      sku_id: item.sku_id,
      sku_name: product?.name ?? item.sku_id,
      ...
      has_esim_profile: profileMap[item.id] ?? false,
      matched_iccid: null,
    }
  })
