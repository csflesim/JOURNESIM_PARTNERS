...
// GET /api/agent/operations/after-sales?agent_id=xxx&...
// 限制只能查該代理商的售後單
export async function GET(req: Request) {
  ...
  // 取得代理商的訂單 channel_order_ids
  const { data: agentOrders } = await supabase
    .from('orders')
    .select('channel_order_id')
    .eq('agent_id', agentId)
  const agentChannelIds = agentOrders?.map(o => o.channel_order_id) ?? []
  if (agentChannelIds.length === 0) return NextResponse.json({ items: [], total: 0 })
  ...
  let query = supabase
    ...
    .select(
      `id, channel_order_id, channel_sub_order_id, bc_after_sale_id,
       ...
       orders(bc_order_id, order_type, order_items(sku_id, products(name)))`,
      { count: 'exact' }
    )
    .in('channel_order_id', agentChannelIds)
  ...
  const items = (data ?? []).map(row => {
    ...
    return { ...row, bc_order_id: order?.bc_order_id ?? null, order_type: order?.order_type ?? null, sku_name: product?.name ?? null }
  })

  return NextResponse.json({ items, total: count ?? 0 })
}

// POST: 查詢 BC 售後詳情（跟 admin 版相同）
