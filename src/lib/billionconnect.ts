...
export interface BCAfterSaleInfo {
  ...
  returnCard?: string
  auditStatus: string   // 0-未審核 1-已撤回 2-審核通過 3-已駁回 4-待修改
  auditOpinion?: string
  refundStatus?: string // 0-待退款 1-已退款 2-已駁回
...
export interface BCDailyTrafficItem {
  usedDate: string           // yyyyMMdd
  type: string               // 0-Data 1-SMS 2-USSD 3-LU
  usedAmount: string         // type=0 單位 KB，其他單位條
  country: string            // 國家名稱
  countryRegionCode: string  // 國家代碼
}
...
export async function getDailyTraffic(params: {
  ...
  tzType?: string      // '0'-運營商時區 '1'-東八區（預設1）
  language?: string    // '1'-中文 '2'-英語
}) {
  return callBC<BCDailyTrafficItem[]>('F023', params)
}

// =====================================================
// F046 — 查詢套餐使用資訊 v2（用量查詢）
// =====================================================
export interface BCPlanUsageV2Sub {
  skuId: string
  skuName: string
  copies: string
  planStatus: string        // 0-未使用 1-使用中 2-已結束 3-已取消
  planStartTime?: string
  planEndTime?: string
  totalDays?: string
  totalTraffic?: string     // KB，-1 不限量
...
export interface BCEsimRechargeResult {
  iccidValidity?: string   // ICCID 有效期
  skuId?: string[]         // 匹配的商品 SKU 列表
}
...
// F054 — 查詢實名認證狀態
// =====================================================
export async function getKycStatus(iccid: string) {
  return callBC<{ status: number; expiryTime?: string }>('F054', { iccid })
}
